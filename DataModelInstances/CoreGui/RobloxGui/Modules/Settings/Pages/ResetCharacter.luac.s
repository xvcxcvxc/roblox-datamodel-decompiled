PROTO_0:
  GETUPVAL R1 0
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K0 ["AnalyticsTargetName"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["AnalyticsInGameMenuName"]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K2 ["AnalyticsRespawnCharacterName"]
  DUPTABLE R6 K5 [{"confirmed", "universeid"}]
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K6 ["AnalyticsCancelledName"]
  SETTABLEKS R7 R6 K3 ["confirmed"]
  GETIMPORT R9 K8 [game]
  GETTABLEKS R8 R9 K9 ["GameId"]
  FASTCALL1 TOSTRING R8 [+2]
  GETIMPORT R7 K11 [tostring]
  CALL R7 1 1
  SETTABLEKS R7 R6 K4 ["universeid"]
  NAMECALL R1 R1 K12 ["SetRBXEventStream"]
  CALL R1 5 0
  GETUPVAL R2 2
  GETTABLEKS R1 R2 K13 ["HubRef"]
  JUMPIFNOT R1 [+8]
  GETUPVAL R2 2
  GETTABLEKS R1 R2 K13 ["HubRef"]
  MOVE R3 R0
  LOADB R4 1
  NAMECALL R1 R1 K14 ["PopMenu"]
  CALL R1 3 0
  RETURN R0 0

PROTO_1:
  GETIMPORT R3 K3 [Enum.UserInputState.Begin]
  JUMPIFNOTEQ R1 R3 [+35]
  LOADB R3 1
  GETTABLEKS R4 R2 K4 ["UserInputType"]
  GETIMPORT R5 K6 [Enum.UserInputType.Gamepad1]
  JUMPIFEQ R4 R5 [+23]
  LOADB R3 1
  GETTABLEKS R4 R2 K4 ["UserInputType"]
  GETIMPORT R5 K8 [Enum.UserInputType.Gamepad2]
  JUMPIFEQ R4 R5 [+16]
  LOADB R3 1
  GETTABLEKS R4 R2 K4 ["UserInputType"]
  GETIMPORT R5 K10 [Enum.UserInputType.Gamepad3]
  JUMPIFEQ R4 R5 [+9]
  GETTABLEKS R4 R2 K4 ["UserInputType"]
  GETIMPORT R5 K12 [Enum.UserInputType.Gamepad4]
  JUMPIFEQ R4 R5 [+2]
  LOADB R3 0 +1
  LOADB R3 1
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K13 ["DontResetCharFunc"]
  MOVE R5 R3
  CALL R4 1 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["DontResetCharFunc"]
  MOVE R2 R0
  CALL R1 1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["LocalPlayer"]
  JUMPIFNOT R0 [+11]
  GETTABLEKS R1 R0 K1 ["Character"]
  JUMPIFNOT R1 [+8]
  LOADK R4 K2 ["Humanoid"]
  NAMECALL R2 R1 K3 ["FindFirstChild"]
  CALL R2 2 1
  JUMPIFNOT R2 [+3]
  LOADN R3 0
  SETTABLEKS R3 R2 K4 ["Health"]
  GETUPVAL R1 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K5 ["AnalyticsTargetName"]
  GETUPVAL R5 2
  GETTABLEKS R4 R5 K6 ["AnalyticsInGameMenuName"]
  GETUPVAL R6 2
  GETTABLEKS R5 R6 K7 ["AnalyticsRespawnCharacterName"]
  DUPTABLE R6 K10 [{"confirmed", "universeid"}]
  GETUPVAL R8 2
  GETTABLEKS R7 R8 K11 ["AnalyticsConfirmedName"]
  SETTABLEKS R7 R6 K8 ["confirmed"]
  GETIMPORT R9 K13 [game]
  GETTABLEKS R8 R9 K14 ["GameId"]
  FASTCALL1 TOSTRING R8 [+2]
  GETIMPORT R7 K16 [tostring]
  CALL R7 1 1
  SETTABLEKS R7 R6 K9 ["universeid"]
  NAMECALL R1 R1 K17 ["SetRBXEventStream"]
  CALL R1 5 0
  GETUPVAL R1 1
  LOADK R3 K18 ["InGameMenu-ResetCharacter"]
  NAMECALL R1 R1 K19 ["ReportCounter"]
  CALL R1 2 0
  RETURN R0 0

PROTO_4:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["ResetBindable"]
  JUMPIFNOTEQKB R0 TRUE [+4]
  GETUPVAL R0 1
  CALL R0 0 0
  JUMP [+10]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["ResetBindable"]
  JUMPIFNOT R0 [+6]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["ResetBindable"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K2 ["HubRef"]
  JUMPIFNOT R0 [+8]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K2 ["HubRef"]
  LOADB R2 0
  LOADB R3 1
  NAMECALL R0 R0 K3 ["SetVisibility"]
  CALL R0 3 0
  RETURN R0 0

PROTO_5:
  GETIMPORT R0 K1 [require]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["Modules"]
  GETTABLEKS R2 R3 K3 ["Settings"]
  GETTABLEKS R1 R2 K4 ["SettingsPageFactory"]
  CALL R0 1 1
  NAMECALL R1 R0 K5 ["CreateNewPage"]
  CALL R1 1 1
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K6 ["DontResetCharFunc"]
  NEWCLOSURE R2 P1
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K7 ["DontResetCharFromHotkey"]
  NEWCLOSURE R2 P2
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K8 ["DontResetCharFromButton"]
  LOADNIL R2
  SETTABLEKS R2 R1 K9 ["TabHeader"]
  GETTABLEKS R2 R1 K10 ["Page"]
  LOADK R3 K11 ["ResetCharacter"]
  SETTABLEKS R3 R2 K12 ["Name"]
  LOADB R2 0
  SETTABLEKS R2 R1 K13 ["ShouldShowBottomBar"]
  LOADB R2 0
  SETTABLEKS R2 R1 K14 ["ShouldShowHubBar"]
  GETUPVAL R2 3
  LOADK R3 K15 ["TextLabel"]
  CALL R2 1 1
  DUPTABLE R3 K26 [{"Name", "Text", "Font", "FontSize", "TextColor3", "BackgroundTransparency", "Size", "TextWrapped", "ZIndex", "Parent", "Position"}]
  LOADK R4 K27 ["ResetCharacterText"]
  SETTABLEKS R4 R3 K12 ["Name"]
  LOADK R4 K28 ["Are you sure you want to reset your character?"]
  SETTABLEKS R4 R3 K16 ["Text"]
  GETUPVAL R5 4
  GETTABLEKS R4 R5 K29 ["font"]
  GETIMPORT R5 K32 [Enum.Font.SourceSansBold]
  LOADK R6 K33 ["Confirmation"]
  CALL R4 2 1
  SETTABLEKS R4 R3 K17 ["Font"]
  GETUPVAL R5 4
  GETTABLEKS R4 R5 K34 ["fontSize"]
  GETIMPORT R5 K36 [Enum.FontSize.Size36]
  LOADK R6 K33 ["Confirmation"]
  CALL R4 2 1
  SETTABLEKS R4 R3 K18 ["FontSize"]
  GETIMPORT R4 K39 [Color3.new]
  LOADN R5 1
  LOADN R6 1
  LOADN R7 1
  CALL R4 3 1
  SETTABLEKS R4 R3 K19 ["TextColor3"]
  LOADN R4 1
  SETTABLEKS R4 R3 K20 ["BackgroundTransparency"]
  GETIMPORT R4 K41 [UDim2.new]
  LOADN R5 1
  LOADN R6 0
  LOADN R7 0
  LOADN R8 200
  CALL R4 4 1
  SETTABLEKS R4 R3 K21 ["Size"]
  LOADB R4 1
  SETTABLEKS R4 R3 K22 ["TextWrapped"]
  LOADN R4 2
  SETTABLEKS R4 R3 K23 ["ZIndex"]
  GETTABLEKS R4 R1 K10 ["Page"]
  SETTABLEKS R4 R3 K24 ["Parent"]
  GETUPVAL R5 5
  JUMPIFNOT R5 [+8]
  GETIMPORT R4 K41 [UDim2.new]
  LOADN R5 0
  LOADN R6 0
  LOADN R7 0
  LOADN R8 100
  CALL R4 4 1
  JUMPIF R4 [+7]
  GETIMPORT R4 K41 [UDim2.new]
  LOADN R5 0
  LOADN R6 0
  LOADN R7 0
  LOADN R8 0
  CALL R4 4 1
  SETTABLEKS R4 R3 K25 ["Position"]
  CALL R2 1 1
  GETUPVAL R3 3
  LOADK R4 K42 ["Frame"]
  CALL R3 1 1
  DUPTABLE R4 K43 [{"Name", "Parent", "Size", "BackgroundTransparency", "Position"}]
  LOADK R5 K44 ["ResetButtonContainer"]
  SETTABLEKS R5 R4 K12 ["Name"]
  SETTABLEKS R2 R4 K24 ["Parent"]
  GETIMPORT R5 K41 [UDim2.new]
  LOADN R6 1
  LOADN R7 0
  LOADN R8 0
  LOADN R9 144
  CALL R5 4 1
  SETTABLEKS R5 R4 K21 ["Size"]
  LOADN R5 1
  SETTABLEKS R5 R4 K20 ["BackgroundTransparency"]
  GETIMPORT R5 K41 [UDim2.new]
  LOADN R6 0
  LOADN R7 0
  LOADN R8 1
  LOADN R9 0
  CALL R5 4 1
  SETTABLEKS R5 R4 K25 ["Position"]
  CALL R3 1 1
  GETUPVAL R4 3
  LOADK R5 K45 ["UIGridLayout"]
  CALL R4 1 1
  DUPTABLE R5 K52 [{"Name", "CellSize", "CellPadding", "FillDirection", "HorizontalAlignment", "SortOrder", "VerticalAlignment", "Parent"}]
  LOADK R6 K53 ["ResetButtonsLayout"]
  SETTABLEKS R6 R5 K12 ["Name"]
  GETUPVAL R7 5
  JUMPIFNOT R7 [+8]
  GETIMPORT R6 K41 [UDim2.new]
  LOADN R7 0
  LOADN R8 44
  LOADN R9 0
  LOADN R10 80
  CALL R6 4 1
  JUMPIF R6 [+7]
  GETIMPORT R6 K41 [UDim2.new]
  LOADN R7 0
  LOADN R8 200
  LOADN R9 0
  LOADN R10 50
  CALL R6 4 1
  SETTABLEKS R6 R5 K46 ["CellSize"]
  GETIMPORT R6 K41 [UDim2.new]
  LOADN R7 0
  LOADN R8 20
  LOADN R9 0
  LOADN R10 20
  CALL R6 4 1
  SETTABLEKS R6 R5 K47 ["CellPadding"]
  GETIMPORT R6 K55 [Enum.FillDirection.Horizontal]
  SETTABLEKS R6 R5 K48 ["FillDirection"]
  GETIMPORT R6 K57 [Enum.HorizontalAlignment.Center]
  SETTABLEKS R6 R5 K49 ["HorizontalAlignment"]
  GETIMPORT R6 K59 [Enum.SortOrder.LayoutOrder]
  SETTABLEKS R6 R5 K50 ["SortOrder"]
  GETIMPORT R6 K61 [Enum.VerticalAlignment.Top]
  SETTABLEKS R6 R5 K51 ["VerticalAlignment"]
  SETTABLEKS R3 R5 K24 ["Parent"]
  CALL R4 1 1
  GETUPVAL R5 6
  NAMECALL R5 R5 K62 ["IsSmallTouchScreen"]
  CALL R5 1 1
  JUMPIFNOT R5 [+14]
  GETIMPORT R5 K64 [Enum.FontSize.Size24]
  SETTABLEKS R5 R2 K18 ["FontSize"]
  GETIMPORT R5 K41 [UDim2.new]
  LOADN R6 1
  LOADN R7 0
  LOADN R8 0
  LOADN R9 100
  CALL R5 4 1
  SETTABLEKS R5 R2 K21 ["Size"]
  JUMP [+6]
  GETUPVAL R5 5
  JUMPIFNOT R5 [+4]
  GETIMPORT R5 K66 [Enum.FontSize.Size48]
  SETTABLEKS R5 R2 K18 ["FontSize"]
  DUPCLOSURE R5 K67 [PROTO_3]
  CAPTURE UPVAL U7
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  LOADB R6 1
  SETTABLEKS R6 R1 K68 ["ResetBindable"]
  NEWCLOSURE R6 P4
  CAPTURE VAL R1
  CAPTURE VAL R5
  GETUPVAL R7 6
  LOADK R9 K11 ["ResetCharacter"]
  LOADK R10 K69 ["Reset"]
  LOADNIL R11
  MOVE R12 R6
  NAMECALL R7 R7 K70 ["MakeStyledButton"]
  CALL R7 5 1
  SETTABLEKS R7 R1 K71 ["ResetCharacterButton"]
  GETTABLEKS R7 R1 K71 ["ResetCharacterButton"]
  LOADNIL R8
  SETTABLEKS R8 R7 K72 ["NextSelectionRight"]
  GETTABLEKS R7 R1 K71 ["ResetCharacterButton"]
  SETTABLEKS R3 R7 K24 ["Parent"]
  GETUPVAL R7 6
  LOADK R9 K73 ["DontResetCharacter"]
  LOADK R10 K74 ["Don't Reset"]
  LOADNIL R11
  GETTABLEKS R12 R1 K8 ["DontResetCharFromButton"]
  NAMECALL R7 R7 K70 ["MakeStyledButton"]
  CALL R7 5 1
  LOADNIL R8
  SETTABLEKS R8 R7 K75 ["NextSelectionLeft"]
  SETTABLEKS R3 R7 K24 ["Parent"]
  GETTABLEKS R8 R1 K10 ["Page"]
  GETIMPORT R9 K41 [UDim2.new]
  LOADN R10 1
  LOADN R11 0
  LOADN R12 0
  GETTABLEKS R15 R7 K76 ["AbsolutePosition"]
  GETTABLEKS R14 R15 K77 ["Y"]
  GETTABLEKS R16 R7 K78 ["AbsoluteSize"]
  GETTABLEKS R15 R16 K77 ["Y"]
  ADD R13 R14 R15
  CALL R9 4 1
  SETTABLEKS R9 R8 K21 ["Size"]
  RETURN R1 1

PROTO_6:
  LOADB R0 1
  SETUPVAL R0 0
  GETUPVAL R0 1
  GETUPVAL R2 2
  GETTABLEKS R1 R2 K0 ["ResetCharacterButton"]
  SETTABLEKS R1 R0 K1 ["SelectedCoreObject"]
  GETUPVAL R0 3
  LOADK R2 K2 ["ResetCharacterAction"]
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K3 ["DontResetCharFromHotkey"]
  LOADB R4 0
  GETIMPORT R5 K7 [Enum.KeyCode.ButtonB]
  NAMECALL R0 R0 K8 ["BindCoreAction"]
  CALL R0 5 0
  RETURN R0 0

PROTO_7:
  LOADB R0 0
  SETUPVAL R0 0
  GETUPVAL R0 1
  LOADK R2 K0 ["ResetCharacterAction"]
  NAMECALL R0 R0 K1 ["UnbindCoreAction"]
  CALL R0 2 0
  RETURN R0 0

PROTO_8:
  JUMPIFNOTEQKB R1 FALSE [+11]
  GETUPVAL R2 0
  JUMPIFNOT R2 [+8]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K0 ["HubRef"]
  LOADNIL R4
  LOADB R5 1
  NAMECALL R2 R2 K1 ["PopMenu"]
  CALL R2 3 0
  GETUPVAL R2 1
  SETTABLEKS R1 R2 K2 ["ResetBindable"]
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [game]
  LOADK R2 K2 ["CoreGui"]
  NAMECALL R0 R0 K3 ["GetService"]
  CALL R0 2 1
  GETIMPORT R1 K1 [game]
  LOADK R3 K4 ["ContextActionService"]
  NAMECALL R1 R1 K3 ["GetService"]
  CALL R1 2 1
  LOADK R4 K5 ["RobloxGui"]
  NAMECALL R2 R0 K6 ["WaitForChild"]
  CALL R2 2 1
  GETIMPORT R3 K1 [game]
  LOADK R5 K7 ["GuiService"]
  NAMECALL R3 R3 K3 ["GetService"]
  CALL R3 2 1
  GETIMPORT R4 K1 [game]
  LOADK R6 K8 ["Players"]
  NAMECALL R4 R4 K3 ["GetService"]
  CALL R4 2 1
  GETIMPORT R5 K1 [game]
  LOADK R7 K9 ["RbxAnalyticsService"]
  NAMECALL R5 R5 K3 ["GetService"]
  CALL R5 2 1
  GETIMPORT R6 K1 [game]
  LOADK R8 K10 ["CorePackages"]
  NAMECALL R6 R6 K3 ["GetService"]
  CALL R6 2 1
  GETIMPORT R7 K12 [require]
  GETTABLEKS R10 R2 K13 ["Modules"]
  GETTABLEKS R9 R10 K14 ["Settings"]
  GETTABLEKS R8 R9 K15 ["Utility"]
  CALL R7 1 1
  GETIMPORT R8 K12 [require]
  GETTABLEKS R11 R2 K13 ["Modules"]
  GETTABLEKS R10 R11 K14 ["Settings"]
  GETTABLEKS R9 R10 K16 ["Theme"]
  CALL R8 1 1
  GETIMPORT R10 K12 [require]
  GETTABLEKS R13 R6 K17 ["Workspace"]
  GETTABLEKS R12 R13 K18 ["Packages"]
  GETTABLEKS R11 R12 K19 ["AppCommonLib"]
  CALL R10 1 1
  GETTABLEKS R9 R10 K20 ["Create"]
  LOADNIL R10
  LOADK R13 K13 ["Modules"]
  NAMECALL R11 R2 K6 ["WaitForChild"]
  CALL R11 2 1
  LOADK R13 K21 ["TenFootInterface"]
  NAMECALL R11 R11 K6 ["WaitForChild"]
  CALL R11 2 0
  GETIMPORT R11 K12 [require]
  GETTABLEKS R13 R2 K13 ["Modules"]
  GETTABLEKS R12 R13 K21 ["TenFootInterface"]
  CALL R11 1 1
  NAMECALL R11 R11 K22 ["IsEnabled"]
  CALL R11 1 1
  GETIMPORT R12 K12 [require]
  GETTABLEKS R13 R2 K13 ["Modules"]
  LOADK R15 K23 ["InGameMenu"]
  NAMECALL R13 R13 K6 ["WaitForChild"]
  CALL R13 2 1
  LOADK R15 K24 ["Resources"]
  NAMECALL R13 R13 K6 ["WaitForChild"]
  CALL R13 2 1
  LOADK R15 K25 ["Constants"]
  NAMECALL R13 R13 K6 ["WaitForChild"]
  CALL R13 2 -1
  CALL R12 -1 1
  DUPCLOSURE R13 K26 [PROTO_5]
  CAPTURE VAL R2
  CAPTURE VAL R5
  CAPTURE VAL R12
  CAPTURE VAL R9
  CAPTURE VAL R8
  CAPTURE VAL R11
  CAPTURE VAL R7
  CAPTURE VAL R4
  MOVE R14 R13
  CALL R14 0 1
  MOVE R10 R14
  LOADB R14 0
  GETTABLEKS R16 R10 K27 ["Displayed"]
  GETTABLEKS R15 R16 K28 ["Event"]
  NEWCLOSURE R17 P1
  CAPTURE REF R14
  CAPTURE VAL R3
  CAPTURE REF R10
  CAPTURE VAL R1
  NAMECALL R15 R15 K29 ["connect"]
  CALL R15 2 0
  GETTABLEKS R16 R10 K30 ["Hidden"]
  GETTABLEKS R15 R16 K28 ["Event"]
  NEWCLOSURE R17 P2
  CAPTURE REF R14
  CAPTURE VAL R1
  NAMECALL R15 R15 K29 ["connect"]
  CALL R15 2 0
  NEWCLOSURE R15 P3
  CAPTURE REF R14
  CAPTURE REF R10
  SETTABLEKS R15 R10 K31 ["SetResetCallback"]
  CLOSEUPVALS R10
  RETURN R10 1
